...
export class InlineCompletionItemProvider implements vscode.InlineCompletionItemProvider {
    provideInlineCompletionItems(
        ...
        console.log('=InlineCompletionTriggerKind', vscode.InlineCompletionTriggerKind.Automatic)
        ...
        results = _getCompletionResults(document, position, context, prefix, suffix);
        if (prefix !== "" && suffix !== "" && results !== undefined && results.length === 0) {
            results = _getCompletionResults(document, position, context, prefix, "");
        }
...
function _getCompletionResults(
    document: vscode.TextDocument,
    position: vscode.Position,
    context: vscode.InlineCompletionContext,
    prefix: string,
